...
1:    @ wait for busy bit to be cleared
	...
	ldr r6,=fanfare_wait_time	@fanfare_kaiで記述されている一音の長さのデータです。
...
@=================================呼び出しした場合のみ動作=======================================
	...
	cmp	r9, #140
	beq	endresult_fanfare	
	add     r6,r6,r5	

	ldr	r1, =fanfare_wait_time
	ldr     r6,[r1, r9]	
	add     r6,r6,r5

	
			@呼び出しタイミングの更新（一音の長さをそれぞれの音に対して配列として格納している場合は読んでこなければいけない）
end_sound:
